...
class CreateArticlesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('articles', function (Blueprint $table) {
            ...
            $table->string('type',50)->comment('属性');
            $table->string('source')->comment('来源');
            $table->unsignedMediumInteger('click')->comment('浏览量');
            $table->unsignedMediumInteger('comment')->comment('评论量');
            $table->unsignedMediumInteger('zan')->comment('赞的量');
            $table->unsignedTinyInteger('status')->default(1)->comment('1：展示，0：不展示');
            $table->string('author',60)->comment('作者');
            $table->string('img')->comment('封面图');
            $table->timestamps();
        });
    }
